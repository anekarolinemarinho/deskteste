...
import { DepartmentId, Department, Submodule } from '../types';
...
const Layout: React.FC<LayoutProps> = ({ children, activeDept, activeSubmodule, onNavigate }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#F8FAFC]">
      <aside className={`bg-navy-gradient text-white hidden md:flex flex-col sticky top-0 h-screen shadow-2xl transition-all duration-300 z-30 ${isCollapsed ? 'w-24' : 'w-72'}`}>
        ...
        <nav className="flex-1 overflow-y-auto px-4 space-y-1 custom-scrollbar pb-10">
          <button 
            onClick={() => { onNavigate('home', null); setExpandedDept(null); }}
